				...
				<div class="content">
					<div class="container-fluid">
						<div class="row">
							<div class="col-md-8">
							<?php
									$query = $this->db->query("SELECT * FROM tanya_jawab JOIN user ON user.id_user=tanya_jawab.id_user WHERE status_komen='0'");
									foreach ($query->result() as $utama):
								?>
							<div class="card">
								<div class="media border p-2 mb-2">
								<img src="<?=base_url()?>upload/user/<?=$utama->gambar?>" alt="foto-user" class="mr-3 mt-3 rounded-circle" style="width:60px;">
								...
								<?php
									...
									$query = $this->db->query("SELECT * FROM tanya_jawab JOIN user ON user.id_user=tanya_jawab.id_user WHERE status_komen='$id_komen' ");
									foreach ($query->result() as $balasan):
								?>
								<div class="container">
								<div class="media border p-2 mb-2">
								<img src="<?=base_url()?>upload/user/<?=$balasan->gambar?>" alt="foto-user" class="mr-3 mt-3 rounded-circle" style="width:60px;">
								<div class="media-body">
								<div class="row">
									<div class="col-sm-10">
								...
								<div id="edit_data_Modal<?php echo $utama->id_komen; ?>" class="collapse" >
									<form class="form-horizontal" method="post" action="<?=base_url()?>C_tanya_jawab/balas">
										...
										<input type="hidden" name="nama_lengkap" class="form-control" value="<?php echo $this->session->userdata('nama_lengkap'); ?>">  
							...
							<div class="card">
								...
								<form method="POST" action="<?=base_url()?>C_tanya_jawab/kirim">
								<input type="hidden" name="id_user" class="form-control" value="<?php echo $this->session->userdata('id_user'); ?>">
								<input type="hidden" name="nama_lengkap" class="form-control" value="<?php echo $this->session->userdata('nama_lengkap'); ?>">  
								<input type="hidden" name="email" class="form-control" value="<?php echo $this->session->userdata('email'); ?>">
								<div class="card-body ">
